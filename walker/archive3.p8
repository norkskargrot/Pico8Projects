...
function tset(x,y,val)
	local byte=terrain[flr(x/4)][y]
	byte=byte&(~(0x.ff<<((2-(x%4))*8)))
	val=val&0x.ff
	local shft=val<<((2-(x%4))*8)
	local newbyte=byte|shft
	terrain[flr(x/4)][y]=newbyte
end

function tget(x,y,ws)
	if ws then
		local tscale=tscale
		local dim=dim
...
function square (dim, x, y, r)
	local avg=tget(x-r,y-r,false)
	avg+=tget(x-r,y+r,false)
	avg+=tget(x+r,y-r,false)
	avg+=tget(x+r,y+r,false)
	avg/=4
	...
	avg=clamp(avg,0,0x.ff)
	tset(x,y,avg)
end

function diamond(dim,x,y,r)
	d1=dim
	avg=tget((x-r)%d1,y,false)
	avg+=tget((x+r)%d1,y,false)
	avg+=tget(x,(y-r)%d1,false)
	avg+=tget(x,(y+r)%d1,false)
	...
	for i=0,dim do
		for j=0,dim do
			...
			if h<waterheight then
				...
				--lighting
				l=tget((i+1)%dim,j,false)
				r=tget((i-1)%dim,j,false)
				light=l-r
				lightcapped=max(min(light,lrange),-lrange)
				mapped=remap(lightcapped,-lrange,lrange,lightmin,lightmax)
				lset(i,j,mapped)
			end
		end
	end
...
function setfill(col,bright)
	if bright<0.5 then
		bright=bright<<5
		color=colours[col][1]&(colours[col][2]<<4)
		fillp(bright|0b.000)
	else
		bright=(bright-0.5)<<5
		color=colours[col][2]&(colours[col][3]<<4)
		fillp(bright|0b.000)
	end	
end
