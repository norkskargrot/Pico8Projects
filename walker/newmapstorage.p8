...
--data
	...
	controlsenabled=true
		--current states:
			--0=normal
			--1=mapopen
	pos={x=tsize/2*tsize,y=tsize/2*tsize,z=1}
	--pos={x=0,y=0,z=1}
	rot=2
...
function _init()
	...
	poke(0x5f36,16)
	--world=genworld(tsize,wnoiscale)
...
function terraintestdraw()
	...
	for i=0,127 do
		for j=0,127 do
			local val=wget(i*tsize,j*tsize)
			val=tgetas(i,j)
			--printh(val)
			pset(i,j,val*16)
		end
...
function wget(x,y)
	...
	local ax=((x/tsize)&0xffff)%tsize
	local ay=((y/tsize)&0xffff)%tsize
	--local byte=world[ax][ay]
	local h=tgetas(ax,ay)
	local h➡️=tgetas(ax+1,ay)
	local h⬇️=tgetas(ax,ay+1)
	local h➡️⬇️=tgetas(ax+1,ay+1)
	if lx+ly<1 then
		local dx=h➡️-h
		local dy=h⬇️-h
...
function tset(x,y,val)
	poke(0x8000+x+y*tsize,val<<8)
end

function tgetas(x,y)
	return peek(0x8000+x+y*tsize)>>8
end

function tgetws(x,y)
 x=((x*tscale)%tsize)&0xffff
 y=((y*tscale)%tsize)&0xffff
	return peek(0x8000+x+y*tsize)>>8
end
...
function openmap(popen,lpopen)
	...
	local l=1000
	local px=(pos.x/tsize)/2+border
	local py=(pos.y/tsize)/2+border
	local lx=((pos.x-l*sin(rot))/tsize)/2+border
	local ly=((pos.y-l*cos(rot))/tsize)/2+border
	line(px,py,lx,ly,8)
	pset(px,py,0)
end
...
function mapposfrompos(posx,posy)
	local x=flr(posx/chunksize)%chunksize
	local y=flr(posy/chunksize)%chunksize
...
function drawlogs()
	...
	print("x=",0,18,0)
	print(pos.x*tscale,8,18,0)
	
	print("y=",0,24,7)
	print(pos.y*tscale,8,24,0)
end
-->8
--sound
function wlksound ()
...
function genobjsinmpos()
	local objs={}
	objs=spawnobjs(objs,7,1)
	objs=spawnobjs(objs,3,2)
...
function spawnobjs (objs,num,ver)
	for k=1,num do
		local newobj={}
		newobj.msx=rnd(chunksize)
		newobj.msy=rnd(chunksize)
		newobj.type=1
		newobj.ver=ver
		objs[#objs+1]=newobj
	end
...
function draworthows(vobjs)
	...
	local pmposx=(pos.x/tsize)%tsize
	local pmposy=(pos.y/tsize)%tsize
...
function updatenpcs()
	local pposx=pos.x
	local pposy=pos.y
	local pmposx=flr((pposx/chunksize))
	local pmposy=flr((pposy/chunksize))
	for i=-1,1 do
		local mposx=((pmposx+i))*chunksize
		for j=-1,1 do
			local mposy=((pmposy+j))*chunksize
			for k,v in pairs(objs[i][j]) do
				v.wx=v.msx+mposx
				v.wy=v.msy+mposy
				if v.type==2 then
