...
--data
	...
	controlsenabled=true
		--current states:
			--0=normal
			--1=mapopen
	pos={x=128/2*128,y=128/2*128,z=1}
	--pos={x=0,y=0,z=1}
	rot=0.1
...
function _init()
	...
	poke(0x5f36,16)
	--world=genworld(128,wnoiscale)
	terrain=gennoise(128,tnoiscale)
	genlighting()
	objs=genobjs()
	initbuilding()
	--dummytdata()
...
function terraintestdraw()

	tset(1,1,0.5)
	for i=0,127 do
		for j=0,127 do
			local val=wget(i*128,j*128)
			val=tgetas(i,j)
			--printh(val)
			pset(i,j,val*16)
		end
...
function getheight(x,y)
	local harea=tgetws(x,y)
	local hwrld=wget(x,y)
	return harea+hwrld*40
end

function wget(x,y)
	local lx=(x%128)/128
	local ly=(y%128)/128
	local ax=((x/128)&0xffff)%128
	local ay=((y/128)&0xffff)%128
	local ax1=ax+1
	local ay1=ay+1
	local h=peek(0x8000+ax+ay*128)>>8
	local h➡️=peek(0x8000+ax1+ay*128)>>8
	local h⬇️=peek(0x8000+ax+ay1*128)>>8
	local h➡️⬇️=peek(0x8000+ax1+ay1*128)>>8
	if lx+ly<1 then
		local dx=h➡️-h
		local dy=h⬇️-h
...
function tset(x,y,val)
	poke(0x8000+x+y*128,val<<8)
end

function tgetas(x,y)
	return peek(0x8000+x+y*128)>>8
end
	...
	--add objects to be drawn
		for k,v in pairs(vobjs) do
			if v.psy<z and v.psy>z-dz then
				...
				v.ssy=(pos.z-height)/v.psy*1000+64
	 ...
	 --screen left in world space
		...
		for i=0,numcols do
			...
			local ssheight=(pos.z-height)/z*1000+64
...
function ttypefromheight(h)
	...
	if h<2 then return 1 end
	h=h/40
	if h<0.15 then return 2 end
	if h<0.5 then return 3 end
	if h<0.8 then return 4 end
	return 5
end
...
function openmap(popen,lpopen)
	...
	local l=1000
	local px=(pos.x/128)/2+border
	local py=(pos.y/128)/2+border
	local lx=((pos.x-l*sin(rot))/128)/2+border
	local ly=((pos.y-l*cos(rot))/128)/2+border
	line(px,py,lx,ly,8)
	pset(px,py,0)
end
...
function mappixelset(i,j,ibord,border)
	local h=tgetas(i*2,j*2)
	local h➡️=tgetas(i*2+1,j*2)
	local h⬇️=tgetas(i*2,j*2+1)
	local ttype=ttypefromheight(h*40)
	local ttypeb=ttypefromheight(h➡️*40)
	local ttypec=ttypefromheight(h⬇️*40)
...
function normalupdate()
 ...
 if btn(4) then
  --strafe
	...
	theight=max(2,getheight(pos.x,pos.y))
...
function mapposfrompos(posx,posy)
	local x=flr(posx/128)%128
	local y=flr(posy/128)%128
...
function	getplayermpos()
	local mpos={}
	mpos.x=flr(pos.x/128)
	mpos.y=flr(pos.y/128)
	return mpos
end
...
function drawlogs()
	...
	print("memory=",0,12,0)
	print(stat(0),28,12,0)
	
	print("x=",0,18,0)
	print(pos.x*0.8,8,18,0)
	
	print("y=",0,24,7)
	print(pos.y*0.8,8,24,0)
end
-->8
--sound
function wlksound ()
...
function draworthops(vobjs)
	...
	for i=0,length-1 do
		...
		v.pixelx=(v.psx)/128*32+64
		v.pixely=(-v.psy)/128*32+64
...
function draworthows(vobjs)
	...
	for i=0,length-1 do
		local v=vobjs[length-i]
		pixelx=(v.wx-pos.x)/128*32+64
		pixely=(v.wy-pos.y)/128*32+64
		rect(pixelx-1,pixely-1,pixelx+1,pixely+1,0)
		pset(pixelx,pixely,v.type*3+v.ver)
	end
	
	local pmposx=(pos.x/128)%128
	local pmposy=(pos.y/128)%128
			
	local xflr=pos.x-flr(pmposx)*128
	local xcel=pos.x-ceil(pmposx)*128
	local yflr=pos.y-flr(pmposy)*128
	local ycel=pos.y-ceil(pmposy)*128
	
	local ⬆️⬅️={}
	⬆️⬅️.x=(128-pos.x)/128*32+64
	⬆️⬅️.y=(128-pos.y)/128*32+64
	local ⬇️➡️={}
	⬇️➡️.x=-pos.x/128*32+64
	⬇️➡️.y=-pos.y/128*32+64
...
function updatenpcs()
	local pposx=pos.x
	local pposy=pos.y
	local pmposx=flr((pposx/128))
	local pmposy=flr((pposy/128))
	for i=-1,1 do
		local mposx=((pmposx+i))*128
		for j=-1,1 do
			local mposy=((pmposy+j))*128
			for k,v in pairs(objs[i][j]) do
				v.wx=v.msx+mposx
				v.wy=v.msy+mposy
				if v.type==2 then
...
function getclippedv(v,dx,dy,clpdst,height)
	...
	newv.ssy=(pos.z-height)/newv.psy*1000+64
	return newv
end
...
function drawwall(v1,v2)
	for ssx=max(0,v1.ssx),min(128,v2.ssx) do
	 ...
	 local y=lerp(percent,v1.wy,v2.wy)
		...
		local ssyb=(pos.z-bot)/psy*1000+64
	...
	local v1px=(v1.psx)/128*32+64
	local v1py=(-v1.psy)/128*32+64
	
	local v2px=(v2.psx)/128*32+64
	local v2py=(-v2.psy)/128*32+64
