...
function getheight(x,y)
	local harea=arrget(terrain,x,y,true)
...
function wget(x,y)
	...
	local ax=flr(x/tsize)%wsize
	local ay=flr(y/tsize)%wsize
	local ax1=(flr(x/tsize)+1)%wsize
	local ay1=(flr(y/tsize)+1)%wsize
	if lx+ly<1 then
		local h=arrget(world,ax,ay,false)
		local dx=arrget(world,ax1,ay,false)-h
		local dy=arrget(world,ax,ay1,false)-h
		return h+lx*dx+ly*dy
	else
		local h=arrget(world,ax1,ay1,false)
		local dx=arrget(world,ax,ay1,false)-h
		local dy=arrget(world,ax1,ay,false)-h
...
function mappixelset(i,j,ibord,border)
	local ttype=ttypefromheight(arrget(world,i,j,false)*whscale)
	local ttypeb=ttypefromheight(arrget(world,min(i+1,wsize),j,false)*whscale)
	local ttypec=ttypefromheight(arrget(world,i,min(j+1,wsize),false)*whscale)
...
function drawobjs()
	local pmpos=getplayermpos()
	for i=-1,1 do
		for j=-1,1 do
			for k,v in pairs(objs[i][j]) do
				local posx=(pmpos.x+i)*tsize+v.x
				local posy=(pmpos.y+i)*tsize+v.y			end
...
__gfx__

__label__
