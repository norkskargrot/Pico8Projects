...
function physupd(p)
	local hit=false
	--slope handling
	local sl,sr=fget(mget((p.x+p.dx)/8,(p.y+p.h)/8),7),fget(mget((p.x+p.dx+p.w)/8,(p.y+p.h)/8),6) 
	if (sl or sr) and p.dy>=0 then
		p.x+=p.dx
		while solidarea(p.x,p.y,p.w,p.h,p) do
			p.y-=1
		end
		if p.rolls then
			if (sl) p.dx+=0.05
		...
		for i=p.x+p.dx,p.x,-(p.dx/abs(p.dx)) do
			if (not solidarea(i,p.y,p.w,p.h,p)) then
				p.x=i
				break
			end
			p.dx=0
	...
	for i=p.y+p.dy,p.y,-(p.dy/abs(p.dy)) do
		if (not solidarea(p.x,i,p.w,p.h,p)) then
			p.y=i
			break
		end
		p.dy=0
	...
	for k,v in pairs(water) do
		if boxboxoverlap(p,v) then
			local k=max(flr((p.x+p.w-v.x)/4),0)
			v.hs[k]+=p.dy+p.dx
			if abs(p.dy)>1 then
				initpart(p.x+p.w/2,p.y+p.h/2,rndz(1),-rnd(1),20,6)
			end
		end
	end
...
function solidarea(x,y,w,h,obj)
	if obj then
		local area={x=x,y=y,w=w,h=h}
		for k,v in pairs(o) do
			if (v!=obj and boxboxoverlap(area,v)) return true
		end
		if (obj!=p and boxboxoverlap(area,p)) return true
	end
	return (issolid(x,y))
	or issolid(x+w,y)
	or issolid(x,y+h)
...
function issolid(x,y)
	...
	--check for slopes
	if (fget(m,7)) return (x%8-y%8<1)
	if (fget(m,6)) return (y%8+x%8>7)	
...
function isgrounded(p)
	return solidarea(p.x,p.y+4,p.w,p.h,p)
end
...
function palways(p)
	...
	if curroom.msgtrig then
		for k,v in pairs(curroom.msgtrig) do
			if boxboxoverlap({x=v.x*8,y=v.y*8,w=v.w*8,h=v.h*8},p) then
				currmsg=v.msg
...
function pnormal(p)
	...
	if btn(2) and fget(tile,3) then
		--climb
		p.update=climbupd
		p.dx,p.dy=0,0
		p.grav=false
	elseif btnd(2) and isgrounded(p) then
		--jump
		p.dy=-2.8;
	...
	--swim
	if fget(tile,2) and p.armour then
		if btn(2) then
	...
	if p.dropping then
		local obj=p.hold
		if not solidarea(obj.x,obj.y,obj.w,obj.h,p) then
			obj.dx=p.dir
			obj.dy=-1
			obj.y-=2
			add(o,obj)
...
function climbupd(p)
	...
	if (btn(3)) p.dy+=0.05
	palways(p)
	if abs(p.dx+p.dy)>0.1 then
		p.dir=1-2*flr(time()*4%2)
	end
	if not fget(getptile(p),3) then
		if (btn(2)) p.dy=-2.2
		p.grav=true
		p.update=pnormal
	end
...
function pattack(p)
	...
	if (p.timer<=0) then
		...
		if not solidarea(newx,p.y,p.w,p.h,p) then
			p.x=newx
		end
		p.spr=0
		p.animoffset=0
...
function pdraw(p)
	...
	--player climbing or in air
	elseif p.update==climbupd
					or not isgrounded(p) then
		spri=4
		
	--player sheilding
	elseif btn(3) and p.sheild then 
		spri=5
...
--water bubbles
	{dx=0,dy=-0.4,rdy=0,w=8,f=30,t=20,col=12},
--torch
	...
	for i=0,room.w-1 do
		for j=0,room.h-1 do
			...
			elseif cur==44 then
				initwater(i,j)
			elseif cur==28 then
				printh("water emitter")
				initmapemitter(i,j-0.2,2)
			--lava
			elseif cur==18 then
...
function initwater(i,j)
	printh(#water)
	for k,v in pairs(water) do
		if (boxpointoverlap({x=i*8,y=j*8+5},v)) return
	end
	local w={}
	w.x,w.y,w.h=i*8,j*8+3,8
	w.hs,w.vs={},{}
	local l=0
	while mget(i+l,j)==44 do
		for k=l*2,l*2+2 do
			w.hs[k]=0
			w.vs[k]=0
		end
...
function updwater(w)
	local const,damp,prop=0.01,0.98,0.05
	for k=0,#w.hs do
		w.vs[k]=w.vs[k]*damp-const*w.hs[k]
		w.hs[k]+=w.vs[k]
		--if (randchance(800)) w.hs[k]+=5
	end
	for k=0,#w.hs do
		local ⬅️,➡️=max(k-1,0),min(k+1,#w.hs)
		w.vs[⬅️]+=(w.hs[k]-w.hs[⬅️])*prop
		w.vs[➡️]+=(w.hs[k]-w.hs[➡️])*prop
	end
end
...
function drawwater(w)
	for k=0,#w.hs-1 do
		local h1,h2=w.y-w.hs[k]+0.5,w.y-w.hs[k+1]+0.5
		for i=0,3 do
			local d,t=w.x+k*4+i,i/3
			local h=t*h2+(1-t)*h1
			line(d,h,d,w.y+4,1)
			pset(d,h,12)
		end
...
tile flags:
	0:solid
	1:renders in front of objs
	2:water (swimmable)
	3:climbable
	6:left facing slope
	7:right-facing slope
]]--	
...
--1:intro forest
	{w=84,h=16,
		...
		msgtrig={
			{x=60,y=11,w=1,h=2,msg="looks like some sort of crypt."},
		},
...
--3:crypt entrance
	{w=32,h=16,
		...
		msgtrig={
			{x=24,y=12,w=2,h=2,msg="i'm definitley not surviing that without some warmer clothes"},
		},
		data="6abe8a7iqe8i7a8i8esi_a-m=arm_#+?kaqi?taa+y+mamtn[qda=aro'aaa:<5caa=?faan?laa+aaa=arm'aqo?)aa7aaa[qda7i8iqe8iqism:0da:#+?laaeaqtn[a-m[mdaaisi7e9o??aaqaaa[<)aamdnaa6i_09o'<pca0+?faaeqeciqecex2ba/a7o'a6o'a-?c#da'0da'#daa#da'i=?fa-paa6p8eto'09o'0+?d#toaa6o?paa'<)aamdaa<+p+a6pqaci:[7h3;rh:0daa09o'aaa'0da+aaa[qda?<dna;9iqa7h(a6lp;sp30to'09o:a6o:#+n[a-m]q+p?ydn/a-m[mdna#+haaqp:#to4#+?c09o:a-maa-maa-paa6p?paa+a6n)&daa[to4;_o:;rh:09iqesiqi8iqice6eci_[rla&+d?paa(#9l>;7l|[9h|09o?pqo'#to'09o?lqo3[rlaa-oa#da|<-oa#9n|09o|&9oa&daa&da'<)a|#to:#th30th30tl5aaa|#th|<5ba#da'<?aa&da?l6o:&to|0+o:<pb&0da",
		exts={
...
function zomupd(e)
	--chnage dir if wall
	if solidarea(e.x+e.dir,e.y,e.w,e.h,e) then
		e.dir=-e.dir
	end
	--chnage dir if edge
...
function barupd(e)
	e.spr=76
	if issolid(e.x+sgn(e.dx),e.y)
	or	issolid(e.x+sgn(e.dx)+e.w,e.y) then
		e.dx=-e.dx*0.8
...

...

__label__
...
__gff__

...

__sfx__
