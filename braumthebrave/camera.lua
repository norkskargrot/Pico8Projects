...
function update_cam()
    ...
    cam.push_bound = cam.x + cam_pos_x - cam.dir * cam_push_bound
    cam.dir_bound_A = cam.x + cam_pos_x - cam.dir * cam_dir_bound
    cam.dir_bound_B = cam.x + cam_pos_x + cam.dir * cam_push_bound
    local room_bound_L = room.x1 * 8
    local room_bound_R = (room.x2 - 7) * 8
    if cam.dir > 0 then
        if p.x >= cam.push_bound then
            cam.goalx = p.x - cam_pos_x + cam.dir * cam_push_bound
        end
        if (p.x <= cam.dir_bound_A) cam.dir = -1
        if (p.x >= cam.dir_bound_B and cam.goalx > room_bound_R) cam.dir = -1
    else
        if p.x <= cam.push_bound then
            cam.goalx = p.x - cam_pos_x + cam.dir * cam_push_bound
        end
        if (p.x >= cam.dir_bound_A) cam.dir = 1
        if (p.x <= cam.dir_bound_B and cam.goalx < room_bound_L) cam.dir = 1
    end
...
function draw_cam_debug()
    --line(p.x, 0, p.x, 127, 12)
    line(cam.dir_bound_A, 0, cam.dir_bound_A, 127, 12)
    line(cam.dir_bound_B, 0, cam.dir_bound_B, 127, 8)
    line(cam.push_bound, 0, cam.push_bound, 127, 4)
end